...
[source,properties]
----
quarkus.camunda.client.broker.gateway-address=localhost:26500
...
`quarkus.camunda.broker.gateway-address`.
...
The Camunda Dev-UI is activated by default. You can disable it by changing this property:
[source,properties]
----
quarkus.camunda.dev-ui.enabled=true
----
...
To use the `CamundaClient` and `BpmnAssert` in the tests use the `@QuarkusTestResource(CamundaTestResource.class)` and enable this configuration:
[source,properties]
----
quarkus.camunda.devservices.enabled=true
----
...
[quote]
The Camunda process test embedded engine requires Java version >= 17
