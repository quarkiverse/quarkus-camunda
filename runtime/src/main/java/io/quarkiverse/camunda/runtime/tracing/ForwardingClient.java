...
import static io.quarkiverse.camunda.runtime.tracing.Tracing.DEPLOY_RESOURCES;
...
import static io.quarkiverse.camunda.runtime.tracing.Tracing.PROCESS_DEF_KEY;
import static io.quarkiverse.camunda.runtime.tracing.Tracing.PROCESS_DEF_VER;
import static io.quarkiverse.camunda.runtime.tracing.Tracing.PROCESS_ELEMENT_INSTANCE_KEY;
import static io.quarkiverse.camunda.runtime.tracing.Tracing.PROCESS_ID;
...
public abstract class ForwardingClient<ReqT, RespT> extends ForwardingClientCall.SimpleForwardingClientCall<ReqT, RespT> {

    public ForwardingClient(ClientCall<ReqT, RespT> delegate) {
        super(delegate);
    }
    ...
    protected void sendTracingMessage(ReqT message, AttributeCallback span, AttributeCallback callSpan) {
        if (message instanceof GatewayOuterClass.CompleteJobRequest) {
            ...
            span.setError().setAttribute(Tracing.FAIL_MESSAGE, r.getErrorMessage());
            ...
            span.setError().setAttribute(Tracing.THROW_ERROR_MESSAGE, r.getErrorMessage())
                    .setAttribute(Tracing.THROW_ERROR_CODE, r.getErrorCode());

            callSpan.setAttribute(JOB_KEY, r.getJobKey())
                    .setAttribute(Tracing.THROW_ERROR_MESSAGE, r.getErrorMessage())
                    .setAttribute(Tracing.THROW_ERROR_CODE, r.getErrorCode());
